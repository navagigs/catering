  ...
  <table width="100%" border="1px">
                                ...
                                ?>
                                    <?php
                                        $no=1;             
                                         ...
                                         LEFT JOIN costumer ON costumer.costumer_id= pembayaran.costumer_id WHERE pembayaran.pembayaran_tanggal between '$dari' and '$sampai'
                                         ORDER BY pembayaran_id DESC");
                                        foreach ($query->result() as $row){ 
                                        ?>
                                <?php
                                $query1 = $this->db->query("SELECT * FROM pembayaran WHERE pembayaran_tanggal between '$dari' and '$sampai' ORDER BY pembayaran_id");
                              ...
                              <?php $no++; } ?>
                                    <tr>
                                        ...
                                        <td><p align="right"><?php   $jumlahkan  = $this->db->query("SELECT SUM(pembayaran_jumlah) AS pembayaran_jumlah FROM pembayaran WHERE pembayaran_tanggal between '$dari' and '$sampai' ");foreach ($jumlahkan->result() as $row2){}
